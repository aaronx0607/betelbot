...
def make_plot(days_ago, dates, mag):
    ...
    plt.text(19.5, mid+1-0.25, "AAVSO 观测星等(肉眼) 蓝色·", color='blue')
    plt.text(19.5, mid+1-0.15, "AAVSO 观测星等(CCD) 黑色×", color='black')
    plt.text(19.5, mid+1-0.05, "局部加权拟合 红色线", color='red')
    plt.text(19.5, mid-1+0.1, '#参宿四的亮度为 ' + lumi + " 等，于 " + date_text + " 由 天文通 用 @betelbot 生成")
