...
def make_plot(days_ago, dates, mag):
    ...
    plt.text(16.5, mid+1-0.25, u"观测星等 蓝色", color='blue', fontdict=fontcn)
    plt.text(13.5, mid+1-0.25, u"○", color='blue', fontdict=fonten)
    plt.text(19.5, mid+1-0.15, u"CCD ", color='black', fontdict=fonten)
    plt.text(16.5, mid+1-0.15, u"观测星等 黑色", color='black', fontdict=fontcn)
    plt.text(13.5, mid+1-0.15, u"×", color='black', fontdict=fonten)
    plt.text(19.5, mid+1-0.05, u"局部加权拟合 红色线", color='red', fontdict=fontcn)
    plt.text(13.5, mid-1+0.1, u'目前参宿四的星等为 ', fontdict=fontcn)
    plt.text(8.5, mid-1+0.1, lumi, fontdict=fonten)
    plt.text(13.5, mid-1+0.2, u" 由 天文通 译制于", fontdict=fontcn)
    plt.text(8.5, mid-1+0.2, date_text, fontdict=fonten)
